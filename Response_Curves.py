...
st.title("Response Curves Generator")
...
with col4:
    step = st.number_input("Step Size", value=1000, step=1000)
...
if uploaded_file:
    try:
        ...
        if {'channel', 'impactable_sensors', 'coefficient', 'spend'}.issubset(model_result_df.columns):
            ...
            model_result_df_display = model_result_df[['channel', 'coefficient', 'impactable%', 'impactable_sensors', 'spend']]
            model_result_df_display['roi'] = (model_result_df_display['impactable_sensors']*price)/model_result_df_display['spend']
            model_result_df_display['Impactable Sales'] = model_result_df_display['impactable_sensors'] * price 

            model_result_df_display = model_result_df_display[['channel','coefficient','impactable%', 'Impactable Sales', 'spend', 'roi']].rename(columns = {
                'channel':'Channel',
            ...
            format_dict = {col: "{:,.2f}" for col in model_result_df_display.select_dtypes(include='number').columns}
            styled_df = model_result_df_display.style.format(format_dict)
            st.dataframe(styled_df)

            selected_roi_df = model_result_df_display[['Channel','ROI']]

            fig = px.bar(selected_roi_df, x = 'Channel', y = 'ROI', title = 'ROI by Channel')
            ...
            if not prefixes:
                ...
                if not all(col in merged_rc.columns for col in required_cols):
                    ...
                    st.subheader("Response Curves Data")
                    
                    filtered_df = merged_rc[[x_col, y_col, mroi_col]]
                    filtered_df['ROI'] = merged_rc[y_col]/merged_rc[x_col]
                    filtered_df = filtered_df[[x_col, y_col, 'ROI', mroi_col]]
                    filtered_df_display = filtered_df.rename(columns = {
                        x_col: 'Spend (in $)',
                        y_col: 'Impactable Sales (in $)',
                        mroi_col : 'mROI'
                    })
                    format_dict_filtered_df = {col: "{:,.2f}" for col in filtered_df_display.select_dtypes(include='number').columns}
                    styled_filtered_df = filtered_df_display.head(1000).style.format(format_dict_filtered_df)
